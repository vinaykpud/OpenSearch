...
public class SearchReplicaAwarenessAllocationIT extends RemoteStoreBaseIntegTestCase {
    ...
    public void testAllocationAwarenessZones() {
        ...
        List<String> nodes = internalCluster().startNodes(
            ...
            Settings.builder().put(commonSettings).put("node.attr.zone", "b").build(),
            Settings.builder().put(commonSettings).put("node.attr.zone", "a").build(),
            Settings.builder().put(commonSettings).put("node.attr.zone", "a").put("node.attr.searchonly", "true").build(),
            Settings.builder().put(commonSettings).put("node.attr.zone", "b").put("node.attr.searchonly", "true").build(),
            Settings.builder().put(commonSettings).put("node.attr.zone", "b").put("node.attr.searchonly", "true").build(),
            Settings.builder().put(commonSettings).put("node.attr.zone", "a").put("node.attr.searchonly", "true").build()
        );
