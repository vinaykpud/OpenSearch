...
public class SearchReplicaAwarenessAllocationTests extends OpenSearchAllocationTestCase {
        ...
        clusterState = ClusterState.builder(clusterState)
            .nodes(
                DiscoveryNodes.builder()
                    ...
                    .add(newNode("node3", Map.of("zone", "a", "searchonly", "true")))
                    .add(newNode("node4", Map.of("zone", "a", "searchonly", "true")))
            )
            .build();
        ...
        clusterState = ClusterState.builder(clusterState)
            .nodes(
                DiscoveryNodes.builder(clusterState.nodes())
                    ...
                    .add(newNode("node6", Map.of("zone", "b", "searchonly", "true")))
            )
            .build();
        clusterState = strategy.reroute(clusterState, "reroute");
    ...
    public void testMoveShardOnceNewNodeWithOutAwarenessAttributeAdded() {
        ...
        clusterState = ClusterState.builder(clusterState)
            .nodes(
                DiscoveryNodes.builder()
                    .add(newNode("node1", Map.of("zone", "a")))
                    .add(newNode("node2", Map.of("zone", "a", "searchonly", "true")))
            )
            .build();
        clusterState = strategy.reroute(clusterState, "reroute");
        assertThat(clusterState.getRoutingNodes().shardsWithState(INITIALIZING).size(), equalTo(1));
        ...
        clusterState = ClusterState.builder(clusterState)
            .nodes(DiscoveryNodes.builder(clusterState.nodes()).add(newNode("node3", Map.of("searchonly", "true"))))
            .build();
    ...
    public void testFullAwarenessWithSearchReplica() {
        ...
        clusterState = ClusterState.builder(clusterState)
            .nodes(DiscoveryNodes.builder(clusterState.nodes()).add(newNode("node3", Map.of("zone", "2", "searchonly", "true"))))
            .build();
        clusterState = strategy.reroute(clusterState, "reroute");
