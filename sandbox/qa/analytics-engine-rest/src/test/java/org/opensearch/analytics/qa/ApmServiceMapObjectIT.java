...
/**
 * Regression contract for the dashboards-observability APM service-map experience.
 *
 * <p>Pins the eight PPL queries in
 * {@code dashboards-observability/public/components/apm/query_services/query_requests/ppl_queries.ts}.
 * Six of them project <em>parent object</em> identifiers ({@code sourceNode.keyAttributes},
 * {@code targetNode.groupByAttributes}, …) and used to fail on the analytics engine with
 * {@code IllegalArgumentException: Field [sourceNode.keyAttributes] not found} — thrown by
 * {@code QualifiedNameResolver} because an {@code object} was not a column in the Calcite row
 * type. Service-map topology, service detail panels, and operation/dependency lists went blank.
 * Only the two {@code distinct_count} widgets (leaf scalars) worked.
 *
 * <p>Now the schema exposes each object as a struct column and {@code ObjectStructMaterializer}
 * assembles it with {@code make_struct} above the scan, so a parent-object identifier resolves
 * for every operator — not just projection. {@link #testOperatorMatrixOnParentObject} covers the
 * operator matrix from the bug report ({@code fields} / {@code eval} / {@code dedup} /
 * {@code sort} / {@code isnotnull}), since the original report noted the failure was broader
 * than projection.
 *
 * <p>Fixture topology (3 connections): frontend→checkout, frontend→payment, checkout→inventory.
 */
