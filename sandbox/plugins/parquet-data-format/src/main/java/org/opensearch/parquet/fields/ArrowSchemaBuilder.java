...
public final class ArrowSchemaBuilder {
    ...
    public static Schema getSchema(MapperService mapperService) {
        ...
        if (documentMapper != null) {
            // Nested (N1): fields under a nested object mapper are packed into a
            // LIST<STRUCT> column on the parent row instead of flat leaf columns.
            ...
            // Emit one LIST<STRUCT> field per TOP-LEVEL nested mapper (nested mappers inside
            // another nested mapper become list fields inside the parent's struct).
    ...
    /**
     * Returns the deepest nested path that strictly contains {@code name}
     * (i.e. {@code name} starts with {@code path + "."}), or null if none.
     */
    ...
    /**
     * Builds the Arrow LIST&lt;STRUCT&gt; field for the nested mapper at {@code path}.
     * Struct children are the mapper's direct leaf fields (named by leaf segment) plus,
     * recursively, one LIST&lt;STRUCT&gt; per directly-contained nested mapper.
     */
     ...
     * canonical parquet MAP layout (verified by design/nested-map-attributes M0 against DF54):
     * {@code Map("key_value": non-null Struct("key": non-null Utf8, "value": Utf8), unsorted)}.
     */
