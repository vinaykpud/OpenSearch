...
public class VSRManager implements AutoCloseable {
    ...
    /**
     * Writes the document's buffered nested children into their
    ...
    private void writeChildList(
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            // map children of this element (e.g. a flat_object `attributes`). Write EVERY map child of the
            // struct — even when this element has no entries for it — so the MAP's list offsets stay
            // contiguous across elements (an untouched offset would corrupt neighbouring elements' ranges).
     ...
     * every top-level MAP vector so each row's list offset is set (empty when the doc has no entries),
     * keeping offsets contiguous across rows.
     */
     ...
     * an empty (non-null) map. Keys/values are stringified to UTF-8 (POC).
     */
