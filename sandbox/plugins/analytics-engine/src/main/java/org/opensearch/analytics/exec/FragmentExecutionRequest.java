 ...
 * the fragment conversion driver. The data node selects the best alternative based on
...
public class FragmentExecutionRequest extends ActionRequest {
    ...
    public ActionRequestValidationException validate() {
        ...
        if (shardId == null) {
            ...
            validationException.addValidationError("shardId must not be null");
        }
        if (planAlternatives == null || planAlternatives.isEmpty()) {
            if (validationException == null) {
                validationException = new ActionRequestValidationException();
            }
            validationException.addValidationError("planAlternatives must not be empty");
        }
    ...
    public static class PlanAlternative {
        ...
        public PlanAlternative(StreamInput in) throws IOException {
            ...
            // Empty byte array on the wire represents null fragmentBytes (writeByteArray cannot write null).
            // This means empty and null are indistinguishable after deserialization, which is acceptable
            // since a zero-length plan is never valid.
