...
public class CompositeDocumentInput implements DocumentInput<List<? extends DocumentInput<?>>> {
    ...
    public void startNestedChild(String nestedPath) {
        // Broadcast the nested-child open to every format so each builds its nested representation from the
        // SAME parse-order signal stream (Lucene opens a child doc; Parquet begins a LIST<STRUCT> element).
